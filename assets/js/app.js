  const STORAGE_KEYS = {
    ...
    issueLog: 'dressi.issueLog',
    settings: 'dressi.settings'
  };
  ...
  function showToast(message, type = 'success') {
    ...
    if (!window.Toastify) {
      console.warn(`[Dressi ${type}] ${message}`);
    ...
    try {
      ...
      console.warn(`[Dressi toast fallback] ${message}`, error);
      showNativeToast(message, type);
    }
  }
  ...
  function readIssueLog() {
    try {
      return JSON.parse(localStorage.getItem(STORAGE_KEYS.issueLog) || '[]');
    } catch (error) {
      console.warn('[Dressi issue log] Could not read saved issue log.', error);
      return [];
    }
  }

  function saveIssueLogEntry(entry) {
    try {
      const issueLog = readIssueLog();
      issueLog.unshift(entry);
      localStorage.setItem(STORAGE_KEYS.issueLog, JSON.stringify(issueLog.slice(0, 50)));
    } catch (error) {
      console.warn('[Dressi issue log] Could not save issue log.', error);
    }
  }

  function clearIssueLog() {
    localStorage.removeItem(STORAGE_KEYS.issueLog);
    console.info('[Dressi issue log] Cleared.');
  }

  function logIssue(title, error, details = {}) {
    const message = issueMessage(error);
    const occurredAt = new Date();
    const entry = {
      date: dateInputValue(occurredAt),
      time: occurredAt.toLocaleTimeString(APP_SETTINGS.locale),
      timestamp: occurredAt.toISOString(),
      title,
      exactIssue: message,
      details,
      error: issueDetails(error),
      page: location.href
    };

    saveIssueLogEntry(entry);
    console.groupCollapsed(`[Dressi issue] ${entry.date} ${entry.time} - ${title}: ${message}`);
    console.info('Saved issue log entry:', entry);
    console.error(error);
  ...
  window.DressiIssueLog = {
    list: readIssueLog,
    clear: clearIssueLog
  };

  window.addEventListener('error', event => {
    logIssue('Unexpected page error', event.error || event.message, {
      file: event.filename,
      line: event.lineno,
      column: event.colno
    });
  ...
  window.addEventListener('unhandledrejection', event => {
    logIssue('Unexpected async error', event.reason);
  });
  ...
  function saveStoredValue(key, value) {
    try {
      ...
      logIssue('Could not save browser storage', error, { storageKey: key });
    }
  }

  function loadAppSettings() {
    const storedSettings = loadStoredValue(STORAGE_KEYS.settings, {});
    const lowStockLimit = Math.max(1, Number(storedSettings.lowStockLimit || APP_SETTINGS.lowStockLimit));

    APP_SETTINGS.lowStockLimit = lowStockLimit;
    window.DressiSettings = {
  ...
  function saveAppSettings() {
    saveStoredValue(STORAGE_KEYS.settings, {
      lowStockLimit: APP_SETTINGS.lowStockLimit
    });
    window.DressiSettings.lowStockLimit = APP_SETTINGS.lowStockLimit;
  }
  ...
  async function loadInventoryFromSupabase() {
    ...
    if (error) {
      logIssue('Could not load dresses from Supabase', error);
      return;
    }
  ...
  async function loadOrdersFromSupabase() {
    ...
    if (error) {
      logIssue('Could not load orders from Supabase', error);
      return;
    }
  ...
  async function loadRemoteData() {
    if (!supabaseClient) {
      logIssue('Supabase is not connected', 'Check your internet connection and reload the page.');
      return;
    }
    await withLoading('Loading shop data...', () => Promise.all([loadInventoryFromSupabase(), loadOrdersFromSupabase()]));
    renderCurrentPage();
  }
  ...
  window.DressiUtils.currency = currency;
  ...
  function bindPlaceOrder() {
    ...
    placeOrderButton.addEventListener('click', async () => {
      ...
      try {
        await withLoading('Placing order...', async () => {
        ...
        saveAppData();
        updateCartCount();
        showToast(`${order.orderNo} placed successfully`, 'success');
        showShopPage();
        ...
        placedOrderCount -= 1;
        logIssue('Could not place order', error);
        renderCartPage();
      }
    });
  }
  ...
  async function returnCartItemToStock(id, quantity = 1) {
    ...
    try {
      await withLoading('Updating cart...', () => updateDress(item));
      updateCartCount();
      renderCartPage();
      renderProducts();
      renderHeroSlider();
      ...
      logIssue('Could not update cart', error, { itemId: id, returnQuantity });
      renderCartPage();
    }
  }
  ...
  async function clearCart() {
    ...
    try {
      await withLoading('Clearing cart...', () => Promise.all(entries.map(([id]) => {
          const item = inventory.find(product => product.id === id);
      ...
      logIssue('Could not clear cart', error);
      renderCartPage();
    }
  }
  ...
  function renderHistoryPage() {
    ...
    const historyRows = filteredHistory.map(order => `
      <article class="history-row">
        <div class="history-cell history-order-cell">
          <span class="order-number">${escapeHtml(order.orderNo)}</span>
          <span>${escapeHtml(order.date)}</span>
        </div>
        <div class="history-cell history-customer-cell">
          <strong>${escapeHtml(order.customerName)}</strong>
          <span>${escapeHtml(order.names || 'No item details recorded')}</span>
        </div>
        <div class="history-cell history-count-cell">
          <strong>${order.items}</strong>
          <span>${order.lineItems.length} styles</span>
        </div>
        <div class="history-cell history-total-cell">
  ...
  async function addToCart(id) {
    ...
    try {
      await withLoading('Adding to cart...', () => updateDress(item));
    } catch (error) {
      item.stock = previousStock;
      ...
      logIssue('Could not add to cart', error, { itemId: id });
      return;
    }
  ...
  function openStockPanel(id = activeStockItemId) {
    ...
    if (stockForm && item) {
      stockForm.addEventListener('submit', async event => {
        ...
        try {
          await withLoading('Saving stock...', () => updateDress(item));
          els.search.value = '';
          activateAllFilter();
          renderProducts();
          renderHeroSlider();
          ...
          logIssue('Could not save stock', error, { itemId: item.id });
        }
      });
    }
    ...
    document.querySelector('#addItemForm').addEventListener('submit', async event => {
      ...
      try {
        ...
        submitButton.disabled = false;
        logIssue('Could not add item', error, { sku });
      }
    });
